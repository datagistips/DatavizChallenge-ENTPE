...
```{r}
library(raster)
r <- raster("../data/BDALTIV2-75M_ASC_LAMB93_FXX_2013-10-01/1_DONNEES_LIVRAISON_2013-10-00130/BDALTIV2_MNT_75M_LAMB93_IGN69_FRANCEMETRO20130920/ta.asc")
```

Ouverture des données friches :
```{r}
library(sf)
friches_points <- st_read("../data/friches_points.gpkg")
```
Extraction des friches qualifiées et reprojection en Lambert
```{r}
library(tidyverse)
friches_points <- friches_points %>% dplyr::filter(source_nom != "Site Basias ou Basol non vérifié par le Cerema") %>% st_transform(2154)
```
