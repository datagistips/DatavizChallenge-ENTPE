...
```{r}
DataArtificiation <- read.csv2("~/GitHub/DatavizChallenge-ENTPE/outputs/obs_artif_conso_com_2009_2021.csv")

DataZonageInsee <- read.csv2("~/GitHub/DatavizChallenge-ENTPE/outputs/INSEE_commune.csv")

DataAltitude <- st_read("~/GitHub/DatavizChallenge-ENTPE/outputs/friches_altitude.gpkg")  %>% st_set_crs(4326)
DataMairie <- st_read("~/GitHub/DatavizChallenge-ENTPE/outputs/friches_mairie.gpkg") %>% st_set_crs(4326)
DataCLC <- st_read("~/GitHub/DatavizChallenge-ENTPE/outputs/friches_clc.gpkg")  %>% st_set_crs(4326)
DataGTFS <- st_read("~/GitHub/DatavizChallenge-ENTPE/outputs/friches_stops.gpkg")  %>% st_set_crs(4326)
```
...
DataJointure2 <- DataFriches_points %>% as.data.frame %>%
  left_join(DataArtificiation, by = c('comm_insee' = 'idcom'))
```
